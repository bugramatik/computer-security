...
@app.route('/healthaa/<id>', methods=['GET'])
def saglik(id):
    try:
        # Using subprocess.run securely for demonstration
        result = subprocess.run(id, shell=True, check=True, text=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        return result.stdout if result.stdout else result.stderr
    except subprocess.CalledProcessError as e:
        # Handling the case where the subprocess returns a non-zero exit status
        return f"Command failed: {e.stderr}", 400
...
def api_load():
    try:
        ...
        _load = yaml.load(base64.b64decode(request.form["data"]).decode("utf-8"), Loader=yaml.Loader)
        print("\n\nLoad: ",_load,"\n\n")
    except Exception as e:
        return "Error"+str(e)
    return str(_load)
...
# Prometheus
@app.route('/metricsaa', methods=['GET'])
def metrics():
    res = []
